...
const CampMap: React.FC<CampMapProps> = ({ camp }) => {
  // Calculate center and bounds
  const { center, bounds, validDropOffLocations } = useMemo(() => {
    const campLat = Number(camp.lat)
    const campLng = Number(camp.lng)
    ...
    const locations: [number, number][] = [[campLat, campLng]]
    
    const validDropOffs = (camp.dropOffLocations || []).filter((loc) => {
  ...
  const campLat = Number(camp.lat)
  const campLng = Number(camp.lng)
  const hasValidCampLocation = !isNaN(campLat) && !isNaN(campLng) && campLat !== 0 && campLng !== 0

  if (!hasValidCampLocation) {
    return (
      <div className="w-full h-64 bg-gray-100 rounded-lg flex items-center justify-center">
        <p className="text-gray-500">Camp location not available</p>
      </div>
    )
  }
